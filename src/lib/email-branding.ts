...
export function buildBrandedEmailShell({
  ...
  return `
    <div style="margin:0;padding:0;background:#F5F8FC;color:#0B1220;font-family:Inter,Arial,sans-serif;">
          ...
          <div style="padding:28px;">
            ...
            ${
              safeCtaHref && safeCtaLabel
                ? `<p style="margin:24px 0 0;"><a href="${safeCtaHref}" style="display:inline-block;border-radius:999px;background:#1E63FF;color:#FFFFFF;font-size:14px;font-weight:700;text-decoration:none;padding:12px 18px;">${safeCtaLabel}</a></p>`
                : ""
            }
          </div>
        </div>
        <div style="padding:16px 4px 0;color:#64748B;font-size:12px;line-height:1.6;">
